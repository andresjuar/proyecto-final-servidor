...
| `googleId`       | `String`, opcional     | único (índice sparse), reservado para login con Google |
...
(`POST /auth/register`). El login con Google (`googleId`) está en el schema
y contemplado en el diseño, pero su implementación real queda pendiente —
las rutas correspondientes existen como dummy.
...
| `owner`          | `ObjectId` → `User`  | se asigna desde el usuario autenticado (JWT), no desde el body |
...
`timeLimitSeconds` (5–120, default 20).
